...
Note: `z.coerce.number()` converts an empty string to `0` (via `Number('')`), not `undefined` — so a plain `.optional()` never actually gets a chance to apply for an empty optional field; the preprocess step converts `''` to `undefined` first so `.optional()` can bypass coercion entirely. The type is derived via `z.input` (not `z.infer`) because `z.coerce.number()`'s input type is `unknown`/`string`, and `useForm`+`zodResolver` need the pre-coercion input shape for the form's `TFieldValues` generic — using `z.infer` (the post-coercion output type, `number`) causes a `zodResolver` type mismatch.
...
Note: same fix as Task 14's `endMonth`/`endYear` — `z.coerce.number()` turns an empty string into `0` (via `Number('')`), not `undefined`, so a plain `.optional()` never gets to apply for an unfilled optional numeric field; the preprocess step converts `''` to `undefined` first. The type uses `z.input` (not `z.infer`) so `useForm`+`zodResolver`'s `TFieldValues` generic matches the pre-coercion input shape `z.coerce.number()` expects, avoiding a resolver type mismatch.
